...
- CLAUDE.md loads into every conversation and is best for always-on project-wide standards. Skills are best for task-specific expertise.
...
Each handles its own specialty. Don't force everything into skills when another option fits better — and you can use multiple at a time. Use skills when you have knowledge that Claude should apply automatically when the topic is relevant, and combine them with other features for comprehensive customization.
...
- Different subagents need different skills (e.g. frontend reviewer vs. backend reviewer)
...
When skills don't work as expected, the problem usually falls into a few predictable categories. This video walks through each one — from skills that don't trigger to priority conflicts to runtime failures — and gives you a systematic troubleshooting approach. You'll also learn about the skills validator tool, which catches structural problems before you spend time debugging other things.
